...
# Set variables
threshold = 0.5
b_amt = 5  # Reduce the blur amount for a clearer background
...
while True:
    ...
    # Create a mask for person and background using Selfie Segmentation
    ...
    # Use the mask to apply the sky background to the frame
    ...
    for (x, y, w, h) in faces:
        ...
        # Calculate the scaling factor for the Fighter Pilot Cockpit Mask based on the face size
        mask_scale_factor = 2.5

        # Calculate the new dimensions for the Fighter Pilot Cockpit Mask
        ...
        # Load the Fighter Pilot Cockpit Mask image
        mask_image = cv2.imread('fighter_pilot_helmet_mask.png', cv2.IMREAD_UNCHANGED)

        # Resize the Fighter Pilot Cockpit Mask image to match the calculated dimensions
        resized_mask = cv2.resize(mask_image, (mask_end_x - mask_x, mask_end_y - mask_y))

        # Create a mask for the Fighter Pilot Cockpit Mask image
        mask = resized_mask[:, :, 3] / 255.0

        # Inverse the mask
        inverse_mask = 1.0 - mask

        # Blend the colors of the Fighter Pilot Cockpit Mask and the frame_roi
    ...
    cv2.imshow("Fighter Pilot Cockpit Mask and Sky Background", frame_with_background)
