...
hat=cv2.imread('Mantika.png')
glass=cv2.imread('Neckless.png')
...
def put_hat(hat, fc, x, y, w, h):
    ...
    for i in range(hat_height):
        for j in range(hat_width):
            for k in range(3):
                if hat[i][j][k] < 235:
                    fc[y + i - int(0.20 * face_height)][x + j][k] = hat[i][j][k]
    return fc
...
def put_glass(glass, fc, x, y, w, h):
    ...
    for i in range(hat_height):
        for j in range(hat_width):
            for k in range(3):
                if glass[i][j][k] < 235:
                    fc[y + i - int(-0.999* face_height)][x + j][k] = glass[i][j][k]
    return fc
