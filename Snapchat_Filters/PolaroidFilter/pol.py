...
def filter(frame, landmarks):
    ...
    Nose_img = cv2.imread("lips.png", -1)
    Crown_img = cv2.imread("bow.png", -1)
    ...
    CrownWd = abs(landmarks.part(26).x - landmarks.part(17).x)+40
    ...
    glassWd = abs(landmarks.part(48).x - landmarks.part(54).x) + 3
    ...
    y1 = int(landmarks.part(52).y) - 2
    y2 = int(y1 + glassHt)
    x1 = int(landmarks.part(62).x - (glassWd / 2))
    x2 = int(x1 + glassWd)
    roi1 = frame[y1:y2, x1:x2]
    roi_bg = cv2.bitwise_and(roi1, roi1, mask=mask_inv)
