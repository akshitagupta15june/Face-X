...
win.iconbitmap(r'Face-Emotions-Recognition\Emotion-recognition-with-GUI\\assets\\Icons8-Ios7-Logos-Google-Drive-Copyrighted.ico') #giving the window an icon
new = 1
...
def videocapture():
    while True: #continuous loop to keep the window running
        ...
        faces = facecascade.detectMultiScale(gray,scaleFactor=1.3, minNeighbors=5) #this helps in finding features and location in our assets - we are passing in our grayscale input, we are scaling down the image which is done with scaleFactor
        ...
        for (x, y, w, h) in faces: #drawing rectangles on the faces detected and also adding text
            ...
            cropped_img = np.expand_dims(np.expand_dims(cv2.resize(roi_gray, (48, 48)), -1), 0) #resizing the inputs in order to get  them in the same shape as the assets on which our assets were trained
