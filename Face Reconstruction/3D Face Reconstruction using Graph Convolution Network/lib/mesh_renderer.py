...
def phong_shader(normals,
  ...
  normals_dot_lights = tf.clip_by_value(
      tf.reduce_sum(tf.expand_dims(normals, axis=1) * directions_to_lights,
                    axis=3), 0.0, 1.0)  # [batch_size, light_count, pixel_count]
  diffuse_output = tf.expand_dims(diffuse_colors, axis=1) * tf.expand_dims(
      normals_dot_lights, axis=3) * tf.expand_dims(light_intensities, axis=2)
  diffuse_output = tf.reduce_sum(diffuse_output,
  ...
  if camera_position is not None:
    ...
    mirror_reflection_direction = tf.nn.l2_normalize(
        2.0 * tf.expand_dims(normals_dot_lights, axis=3) *
        tf.expand_dims(normals, axis=1) - directions_to_lights,
        axis=3)
    direction_to_camera = tf.nn.l2_normalize(camera_position - pixel_positions,
                                             axis=2)
    ...
    specular_output = tf.expand_dims(specular_colors,
                                     axis=1) * specularity * tf.expand_dims(
...
def tone_mapper(image, gamma):
  ...
  Args:
    image: 4-D float32 tensor with shape [batch_size, image_height,
        image_width, channel_count]. The batch of assets to tone map.
  ...
  Returns:
    ...
    channel_count]. Contains the gamma-corrected assets, clipped to the range
    [0, 1].
  """
  batch_size = image.shape[0].value
  corrected_image = tf.pow(image, gamma)
  image_max = tf.reduce_max(tf.reshape(corrected_image, [batch_size, -1]),
                            axis=1)
...
def mesh_renderer(vertices,
  ...
  if camera_position.get_shape().as_list() == [3]:
    camera_position = tf.tile(tf.expand_dims(camera_position, axis=0),
                              [batch_size, 1])
  ...
  if camera_lookat.get_shape().as_list() == [3]:
    camera_lookat = tf.tile(tf.expand_dims(camera_lookat, axis=0),
                            [batch_size, 1])
  ...
  return renders, clip_space_vertices
